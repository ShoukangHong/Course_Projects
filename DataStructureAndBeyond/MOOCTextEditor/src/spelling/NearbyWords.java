...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for (int i = 0; i <= s.length(); i++){
			...
			for (int charCode = (int)'a'; charCode <= (int)'z'; charCode++){
				String sb = part1 + (char)charCode + part2;
				if(!currentList.contains(sb.toString()) &&
						(!wordsOnly||dict.isWord(sb.toString())) &&
						!s.equals(sb.toString())) {
					currentList.add(sb.toString());
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		// insert first node
		queue.addAll(distanceOne(word, false));
		retList.addAll(distanceOne(word, true));
		visited.add(word);
		int count = 0;
		// TODO: Implement the remainder of this method, see assignment for algorithm
		while (retList.size() < numSuggestions & count < 1000000){
			count += 1;
			String current = queue.remove(0);
			if (visited.contains(current)){continue;}
			visited.add(current);
			for (String validWord: distanceOne(current, true)){
				if (!retList.contains(validWord)){
					retList.add(validWord);
				}
			}
			queue.addAll(distanceOne(current, false));
		}
